#!/usr/bin/env python3
"""
mn-pay-hub/scripts/search-lever.py
Lever job board scraper — Minnesota edition.
...
Run: python3 ~/mn-pay-hub/scripts/search-lever.py
"""
...
LOG_FILE  = os.path.expanduser("~/mn-pay-hub/scripts/lever.log")
LOCK_FILE = os.path.expanduser("~/mn-pay-hub/scripts/.lever.lock")
...
SEED_SLUGS = load_lever_seeds('mn')
SEED_SLUGS = list(dict.fromkeys(SEED_SLUGS))


DISCOVERY_QUERIES = [
    'site:jobs.lever.co "Minnesota" OR "Minneapolis" OR "St. Paul" OR "Twin Cities" salary range 2025 2026',
    'site:jobs.lever.co "Minnesota" OR "Minneapolis" OR "St. Paul" OR "Twin Cities" engineer OR analyst OR manager salary',
    'site:jobs.lever.co "Minnesota" OR "Minneapolis" OR "St. Paul" OR "Twin Cities" "pay range" 2025 2026',
]
...
def is_mn_location(location_str: str, desc_text: str = "") -> bool:
...
def main():
    ...
    for slug in all_slugs:
        ...
        for job in jobs:
            ...
            if not is_mn_location(loc_name, desc_plain):
                continue
            region_count += 1
        ...
        log(f"  MN: {region_count} | New w/ salary: {found_this}")
    ...
    if newly_qualified:
        ...
        try:
            ...
            for slug, count in sorted(newly_qualified.items(), key=lambda x: -x[1]):
                if f'"{slug}"' in source:
                    continue
                log(f"  + {slug} ({count} MN+salary jobs)")
                new_lines.append(f'    "{slug}",  # auto-discovered {TODAY} — {count} MN+salary')
            if new_lines:
                insert_block = "\n".join(new_lines)
                marker = "\nSEED_SLUGS = list(dict.fromkeys(SEED_SLUGS))"
                if marker in source:
