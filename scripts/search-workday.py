#!/usr/bin/env python3
"""
mn-pay-hub/scripts/search-workday.py
Workday API scraper — Minnesota edition.

MN Pay Transparency Act (effective Jan 1, 2025)

Strategy:
  1. Tenant discovery — Exa search finds *.myworkdayjobs.com URLs → extract Minnesota employers
  2. CXS JSON API   — paginate all jobs per employer, filter Minnesota (fast, no JS)
  3. HTML job page  — salary in raw HTML; pure regex, no LLM

Run: python3 ~/mn-pay-hub/scripts/search-workday.py
"""
...
LOG_FILE      = os.path.expanduser("~/mn-pay-hub/scripts/workday.log")
LOCK_FILE     = os.path.expanduser("~/mn-pay-hub/scripts/.workday.lock")
LOOKBACK_DATE = (date.today() - timedelta(days=60)).isoformat() + "T00:00:00.000Z"
LARGE_TENANT_THRESHOLD = 500
REGION_SEARCH_TEXT = "Minnesota"
...
DISCOVERY_QUERIES = [
    'site:myworkdayjobs.com "Minnesota" salary 2026',
    'site:myworkdayjobs.com "Minneapolis" OR "Saint Paul" salary "$" 2026',
    'site:myworkdayjobs.com "Minneapolis" tech OR finance OR healthcare salary 2026',
    'site:myworkdayjobs.com Minnesota retail OR medical devices OR insurance salary 2026',
    'site:myworkdayjobs.com "Bloomington" OR "Eden Prairie" OR "Eagan" MN salary 2026',
]
...
def is_minnesota(locations_text, external_path=""):
    """Return True only if the job is plausibly located in Minnesota.
...
def main():
    ...
    for host, company_id, tenant, company_name in all_tenants:
        ...
        while offset // limit < max_pages:
            ...
            for p in postings:
                if is_minnesota(p.get("locationsText", ""), p.get("externalPath", "")):
                    mn_jobs.append(p)
            offset += limit
        ...
        log(f"  Minnesota jobs: {len(mn_jobs)}")

        # Fetch HTML for each BC job and extract salary
        for i, posting in enumerate(mn_jobs, 1):
    ...
    for index, (url, meta) in enumerate(fallback_candidates, 1):
        ...
        if not is_minnesota(location, external_path):
            continue
