#!/usr/bin/env python3
"""
mn-pay-hub/scripts/search-greenhouse.py
Greenhouse job board scraper — Minnesota edition.
...
Run: python3 ~/mn-pay-hub/scripts/search-greenhouse.py
"""
...
LOG_FILE  = os.path.expanduser("~/mn-pay-hub/scripts/greenhouse.log")
LOCK_FILE = os.path.expanduser("~/mn-pay-hub/scripts/.greenhouse.lock")
...
SEED_SLUGS = load_greenhouse_seeds('mn')
...
def is_mn_job(title: str, location: str, content: str) -> bool:
...
def fetch_company_jobs(slug: str, company_name_override=None):
    ...
    for j in jobs_raw:
        ...
        if not is_mn_job(title, location, content_text):
            continue
...
def main():
    ...
    log("=== MN Greenhouse scraper started ===")
    ...
    log(f"=== Done. {new_count} new MN jobs written to {OUTPUT_FILE} ===")
