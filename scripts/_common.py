#!/usr/bin/env python3
"""
mn-pay-hub/scripts/_common.py
Shared utilities for all Minnesota Pay Hub search scripts.
Minnesota Pay Transparency Act (Minn. Stat. §181.173) — effective Jan 1, 2025 — employers with 30+ employees must include starting salary range + benefits description
"""
...
OUTPUT_FILE = os.path.join(SHARED_DIR, f"mn-jobs-raw-{TODAY}.txt")
DATA_FILE   = os.path.expanduser("~/mn-pay-hub/data/jobs.json")
CURRENCY    = "USD"
REGION      = "MN"
...
REGION_TERMS = ['minnesota', 'minneapolis', 'st. paul', 'saint paul', 'bloomington, mn', 'plymouth, mn', 'brooklyn park', 'maple grove', 'eden prairie', 'woodbury, mn', 'burnsville', 'eagan', 'minnetonka', 'richfield, mn', 'roseville, mn', 'st. cloud', 'duluth, mn', 'rochester, mn', 'mankato', ', mn', 'mn,', 'twin cities', 'the cities']

_NON_REGION_LOC_TERMS = ['toronto', 'vancouver', 'ontario, canada', 'british columbia', 'canada,', 'washington, dc', 'washington dc', 'denver, co', 'boulder, co', 'seattle, wa', 'washington state', 'new york city', 'new york, ny', 'san francisco, ca', 'los angeles, ca', 'chicago, il', 'boston, ma', 'newark, nj', 'baltimore, md', 'burlington, vt', 'bloomington, in']
...
EXTRACT_PROMPT = """\
Extract ONE Minnesota job posting from the text below.
...
Return ONLY valid JSON in this exact format if a valid Minnesota job with explicit USD salary range is found:
{"role":"Job Title","company":"Company Name","min":80000,"max":120000,"location":"Minnesota location","source_url":"{url}","posted":"YYYY-MM-DD"}
